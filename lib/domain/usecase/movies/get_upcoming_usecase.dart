...
class GetUpcomingUsecase
    implements BaseUseCase<GetUpcomingUsecaseInput, List<MovieEntity>> {
  final Repository _repository;

  GetUpcomingUsecase(this._repository);

  @override
  Future<Either<Failure, List<MovieEntity>>> execute(
      GetUpcomingUsecaseInput input) async {
    return await _repository.upcomingMovies(
      page: input.page,
...
class GetUpcomingUsecaseInput extends BaseMovieUseCaseInput {
  final int? page;
  final String? language;

  GetUpcomingUsecaseInput({
    this.page,
    this.language,
